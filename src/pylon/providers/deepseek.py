...
def _extract_usage(usage: Any) -> TokenUsage:
    """Extract token usage from DeepSeek response with cache tracking."""
    return TokenUsage(
...
class DeepSeekProvider:
    ...
    Supports reasoning_content extraction for deepseek-reasoner models
    and prompt_cache_hit_tokens for cache tracking.
    ...
    async def chat(self, messages: list[Message], **kwargs: Any) -> Response:
        ...
        if not result.choices:
            raise ProviderError("DeepSeek returned no choices", details={"model": result.model})
        choice = result.choices[0]
        content = choice.message.content or ""

        # Extract reasoning_content for deepseek-reasoner models
        reasoning = None
    ...
    async def stream(
        ...
        """Stream a chat response from DeepSeek API."""
        api_messages = _to_openai_messages(messages)
        for key in _PYLON_INTERNAL_KWARGS:
            kwargs.pop(key, None)
