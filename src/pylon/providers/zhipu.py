"""Zhipu AI / GLM LLM Provider (OpenAI-compatible API)."""
...
def _extract_usage(usage: Any) -> TokenUsage:
    """Extract token usage from Zhipu response."""
    return TokenUsage(
...
class ZhipuProvider:
    ...
    and web_search tool type in tools array.

    Usage:
        provider = ZhipuProvider(model="glm-5")
    ...
    async def stream(
        ...
        """Stream a chat response from Zhipu API."""
        api_messages = _to_openai_messages(messages)
        for key in _PYLON_INTERNAL_KWARGS:
            kwargs.pop(key, None)
