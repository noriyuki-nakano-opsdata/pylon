"""Moonshot AI / Kimi LLM Provider (OpenAI-compatible API)."""
...
def _extract_usage(usage: Any) -> TokenUsage:
    """Extract token usage from Moonshot response."""
...
class MoonshotProvider:
    ...
    Supports reasoning_content extraction for thinking mode
    and $web_search built-in tool support.

    Usage:
        provider = MoonshotProvider(model="kimi-k2.5")
    ...
    def __init__(
        ...
        *,
        ...
        # Auto-detect endpoint from API key prefix
        if base_url is None:
        ...
        # Kimi Coding endpoint requires specific User-Agent
        client_kwargs: dict[str, Any] = {
            "api_key": api_key,
            "base_url": base_url,
        }
        if "kimi.com/coding" in base_url:
            client_kwargs["default_headers"] = {"User-Agent": "kimi-cli/pylon"}
    ...
    async def chat(self, messages: list[Message], **kwargs: Any) -> Response:
        """Send a chat request to Moonshot API."""
        ...
        try:
            ...
            raise ProviderError(
                f"Moonshot API error: {e}",
                details={"status_code": getattr(e, "status_code", None)},
            ) from e

        if not result.choices:
            raise ProviderError(
                "Moonshot returned no choices",
        ...
        if reasoning_content:
            reasoning = ReasoningOutput(content=reasoning_content)
    ...
    async def stream(
        ...
        """Stream a chat response from Moonshot API."""
        api_messages = _to_openai_messages(messages)
        for key in _PYLON_INTERNAL_KWARGS:
            kwargs.pop(key, None)
