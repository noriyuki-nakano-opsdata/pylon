...
class XAIProvider:
    ...
    Supports reasoning_content extraction for reasoning models.

    Usage:
        provider = XAIProvider(model="grok-4")
    ...
    async def chat(self, messages: list[Message], **kwargs: Any) -> Response:
        """Send a chat request to xAI API."""
        ...
        if not result.choices:
            raise ProviderError("xAI returned no choices", details={"model": result.model})
        choice = result.choices[0]
        content = choice.message.content or ""

        # Extract reasoning_content for reasoning models
        reasoning = None
    ...
    async def stream(
        ...
        """Stream a chat response from xAI API."""
        api_messages = _to_openai_messages(messages)
        for key in _PYLON_INTERNAL_KWARGS:
            kwargs.pop(key, None)
