...
def _normalize_tool_choice(tool_choice: str | None) -> str | None:
    """Normalize Mistral tool_choice values to OpenAI-compatible format.

    Mistral uses "any" to mean "must call a tool", which maps to "required"
    in the OpenAI API.
    """
    if tool_choice == "any":
        return "required"
...
class MistralProvider:
    ...
    Handles tool_choice "any" -> "required" mapping.
    ...
    async def chat(self, messages: list[Message], **kwargs: Any) -> Response:
        ...
        tool_choice = _normalize_tool_choice(kwargs.get("tool_choice"))
        if tool_choice:
        ...
        if not result.choices:
            raise ProviderError("Mistral returned no choices", details={"model": result.model})
    ...
    async def stream(
        ...
        """Stream a chat response from Mistral API."""
        api_messages = _to_openai_messages(messages)
        for key in _PYLON_INTERNAL_KWARGS:
            kwargs.pop(key, None)
