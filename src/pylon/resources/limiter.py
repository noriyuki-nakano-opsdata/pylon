...
class CompositeLimit:
    ...
    def allow(self, now: float | None = None) -> bool:
        """Returns True only if ALL limiters allow. Uses check-then-consume to avoid partial state."""
        # Phase 1: check all limiters without consuming
        for limiter in self._limiters:
            if isinstance(limiter, TokenBucket):
                if limiter.available(now) < 1.0:
                    return False
            elif isinstance(limiter, SlidingWindow):
                if limiter.count(now) >= limiter.max_requests:
                    return False
        # Phase 2: all passed, now consume
        for limiter in self._limiters:
            if isinstance(limiter, TokenBucket):
